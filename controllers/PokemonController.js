...
const create = async(req, res) => {
  try{
    ...
    return res.status(201).json({ message: "Pokemon created successfully", pokemon });
  }catch(err){
    console.error(err);
...
const update = async(req, res) => {
  try{
    ...
    const pokemon = await pokemonModel.findByIdAndUpdate(req.params.id,
      { 
        name, 
        type, 
        EvolutionLvl, 
...
const destroy = async(req, res) => {
  const pokemon = await pokemonModel.findByIdAndDelete(req.params.id, { new: true, runValidators: true });
  if (!pokemon) return res.status(404).json({ message: 'Pokemon not found' });
